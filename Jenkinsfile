...
def slack_send_summary(testlog, test_failed) {
  ...
  def title_string     =  "\"title\":\"Consumer API Integration test results for branch ${BRANCH_NAME}:\"";
          ...
          steps {
            dir('_integration_tests') {
              ...
              script {
                ...
                def management_api_mode = 'CONSUMER_API_ACCESS_TYPE=internal ';
                def junit_report_path = 'JUNIT_REPORT_PATH=consumer_api_test_results_mysql.xml';
                ...
                docker.image('mysql:5').withRun("${mysql_settings}") { c ->
                  ...
                  docker.image("node:${NODE_VERSION_NUMBER}").inside("--link ${c.id}:${mysql_host} --env HOME=${WORKSPACE} --env ConnectionStrings__StatePersistence='${mysql_connection_string}'") {
                    mysql_exit_code = sh(script: "${npm_test_command} --colors --reporter mocha-jenkins-reporter --exit > consumer_api_test_results_mysql.txt", returnStatus: true);

                    mysql_testresults = sh(script: "cat consumer_api_test_results_mysql.txt", returnStdout: true).trim();
                    junit 'consumer_api_test_results_mysql.xml'
                  }
                }

                sh('cat consumer_api_test_results_mysql.txt');
          ...
          steps {
            dir('_integration_tests') {
              ...
              script {
                ...
                def node_env_settings = "${node_env} ${consumer_api_mode} ${junit_report_path} ${config_path}"
                ...
                docker.image('postgres:11').withRun("${postgres_settings}") { c ->
                  ...
                  docker.image("node:${NODE_VERSION_NUMBER}").inside("--link ${c.id}:${postgres_host} --env PATH=$PATH:/$WORKSPACE/node_modules/.bin") {
                    ...
                    postgres_exit_code = sh(script: "${npm_test_command} --colors --reporter mocha-jenkins-reporter --exit > consumer_api_test_results_postgres.txt", returnStatus: true);

                    postgres_testresults = sh(script: "cat consumer_api_test_results_postgres.txt", returnStdout: true).trim();
                    junit 'consumer_api_test_results_postgres.xml'
                  };
                };

                sh('cat consumer_api_test_results_postgres.txt');
            ...
            dir('_integration_tests') {
              ...
              script {
                ...
                docker.image("node:${NODE_VERSION_NUMBER}").inside("--env PATH=$PATH:/$WORKSPACE/node_modules/.bin") {
                  sqlite_exit_code = sh(script: "${npm_test_command} --colors --reporter mocha-jenkins-reporter --exit > consumer_api_test_results_sqlite.txt", returnStatus: true);

                  sqlite_testresults = sh(script: "cat consumer_api_test_results_sqlite.txt", returnStdout: true).trim();
                  junit 'consumer_api_test_results_sqlite.xml'
                };

                sh('cat consumer_api_test_results_sqlite.txt');
